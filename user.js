...
const create = (request, response) => {
    ...
    pool.query('SELECT Count(*) as total FROM masdex_users_all WHERE username = $1 and is_delete=false ', [username], (error, results) => {
        ...
        if (results.rows[0].total > 0) {
            ...
            let complete_path = 'https://api-insafmasdex.disnavpriok.id/api/V1/dokumens/user/' + name
            if (request.files) {
                let sampleFile = request.files.photo;
                console.log(sampleFile);
                const now = Date.now()
                name = now + '_' + sampleFile['name'].replace(/\s+/g, '')
                complete_path = 'https://api-insafmasdex.disnavpriok.id/api/V1/dokumens/user/' + name;
...
const readall = (request, response) => {
    ...
    pool.query('SELECT count(*) as total from tbl_users', (error, results) => {
        ...
        pool.query("SELECT u.*, u.role_id, r.role, 'internal' as type from tbl_users u left join tbl_role r on r.id = u.role_id WHERE u.is_delete=$1", [false], (error, results1) => {
            //bcrypt.compare(password, results.rows[0].password, function(err, res) {
