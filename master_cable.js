...
const read_by_voyage_detail = (request, response) => {
    ...
    pool.query('SELECT count(*) as total FROM tbl_insaf_master_cable where id=$1 and is_delete=false', [id], (error, results) => {
     ...
     var sql= 'SELECT c.id, c.voyage_id, v.pkk_id, c.created_at as date, k.ship_name, k.imo, k.call_sign, k.gt, k.mmsi, k.flag, k.length, k.width, c.status_bernavigasi, c.degree1 as lat_deg, c.minute1 as lat_min, c.second1 as lat_sec, c.direction1 as lat_dir, c.degree2 as long_deg, c.minute2 as long_min, c.second2 as long_sec, c.direction2 as long_dir, jb.jenis_berita, c.more_information, c.berita, c.preamble, c.ck, c.kurs_tengah, c.tagihan_lsc, c.tagihan_llc, c.gf_llc, c.gf_lsc, c.gf_total, c.total_tagihan, c.is_delete, p.perusahaan_pelayaran_id, pp.nama_lengkap as perusahaan_pelayaran, c.user_agen as user_agen_id, c.company_agen as perusahaan_agen_id, pa.nama_lengkap as perusahaan_agen, c.communication_station, p.pelabuhan_asal, p.pelabuhan_tujuan, p.pelabuhan_selanjutnya  FROM tbl_insaf_master_cable c   left join tbl_insaf_jenis_berita jb on jb.id = c.jenis_berita   left join tbl_insaf_voyage v on v.id = c.voyage_id  left join tbl_masdex_kapal k on v.mmsi = k.mmsi  left join masdex_pkk p on v.pkk_id = p.id  left join tbl_stakeholders pp on p.perusahaan_pelayaran_id = pp.id  left join tbl_stakeholders pa on c.company_agen = pa.id  where c.id=$1 and c.is_delete=false group by c.id, c.voyage_id, v.pkk_id, c.created_at, k.ship_name, k.imo, k.call_sign, k.gt, k.mmsi, k.flag, k.length, k.width, c.status_bernavigasi, c.degree1, c.minute1, c.second1, c.direction1, c.degree2, c.minute2, c.second2, c.direction2, jb.jenis_berita, c.more_information, c.berita, c.preamble, c.ck, c.kurs_tengah, c.tagihan_lsc, c.tagihan_llc, c.total_tagihan, c.is_delete, p.perusahaan_pelayaran_id, pp.nama_lengkap, c.user_agen, c.company_agen, pa.nama_lengkap, c.communication_station, p.pelabuhan_asal, p.pelabuhan_tujuan, p.pelabuhan_selanjutnya order by c.id DESC'
     pool.query(sql,[id] ,(error, results) => {
       if (error) {
         throw error
       }
