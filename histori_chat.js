...
const read = (request, response) => {
    ...
    pool.query('SELECT count(*) as total FROM tbl_masdex_chats ', (error, results) => {
      if (error) {
        throw error
      }
...
const read_by_id = (request, response) => {
    ...
    pool.query('SELECT count(*) as total FROM tbl_masdex_chats where roomname=$1 ', [roomname], (error, results) => {
      if (error) {
        throw error
      }
     //console.log(results.rows[0].total)
