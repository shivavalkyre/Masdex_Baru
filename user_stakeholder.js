            ...
            bcrypt.genSalt(10,function(err, res) {
                ...
                bcrypt.hash(password, salt,function(err,res){
                    ...
                    console.log(password_hash);
                     pool.query('INSERT INTO tbl_user_stakeholders (username,password,email,photo) VALUES($1,$2,$3,$4)',[username,password_hash,email, name] ,(error, results) => {
                    if (error) {
                        throw error
                    }
                   ...
                   pool.query('SELECT * FROM tbl_user_stakeholders WHERE username = $1',[username] ,(error, results) => {
                    ...
                    if (error) {
                        throw error
                            ...
                            pool.query('UPDATE tbl_user_stakeholders SET username=$1,password=$2,email=$3,photo=$4 WHERE username=$5',[username,password_hash,email, name,username] ,(error, results) => {
                            if (error) {
                                throw error
                            }                          
