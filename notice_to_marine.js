...
const create = (request, response) => {
    ...
    if (request.files) {
        ...
        complete_path = 'https://api-insafmasdex.disnavpriok.id/api/V1/dokumens/ntm/' + name;
...
const read_new_notif = (request,response)=>{
  ...
  pool.query('SELECT count(*) as total FROM tbl_insaf_notice_to_mariner where is_delete=false and new_msg=true', (error, results) => {
    if (error) {
      throw error
    }
   //console.log(results.rows[0].total)
    ...
    if (error) {
      throw error
        ...
        pool.query('SELECT * FROM tbl_insaf_notice_to_mariner where id=$1 and is_delete=false',[id] ,(error, results) => {
          ...
          const update_time = new Date;
          pool.query('UPDATE tbl_insaf_notice_to_mariner SET new_msg=$1,updated_at=$2 where id=$3'
